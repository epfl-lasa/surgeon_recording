...
# Filter outliers
cleantpsDF = remove_outlier_hampel(cleantpsDF)
print(cleantpsDF.head())
print(cleantpsDF.tail())
...
# Use bias from start
measure_bias_start = 20 # seconds
measure_bias_end = 120 # seconds
unbiasedDF = remove_bias(cleantpsDF, time_to_measure_bias=[20,120])
plot_tpsDF(unbiasedDF, title_str=f'BIAS from {measure_bias_start}s to {measure_bias_end}s TPS', time_for_plot='relative time', nb_rec_channels=6, show_plot=False)

# Use bias from end
measure_bias_start = 550 # seconds
measure_bias_end = 600 # seconds
unbiasedDF2 = remove_bias(cleantpsDF, time_to_measure_bias=[560,600], show_values=True)
