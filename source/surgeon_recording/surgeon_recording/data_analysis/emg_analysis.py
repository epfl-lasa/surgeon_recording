...
data_dir = r'C:/Users/cabasse/Documents/surgeon_recording/source/surgeon_recording/surgeon_recording/emg_recordings/21-12-2022/'
...
interp_calib = myfct.interpolate_clean_emg(cleanemg_calib, start_idx=50)
...
#--
butt_calib = myfct.butterworth_filter(interp_calib)
# butt = myfct.butterworth_filter(interpDF)
# myfct.plot_emgDF(butt_calib)
# myfct.plot_emgDF(butt)

rms_calib = myfct.rms_filter(cleanemg_calib)
...
for col in range (2, rms_calib.shape[1]):
    max_col = rms_calib.iloc[1000:, col].nlargest(3) #finds the 3 highest values of each column
    i_max_col = max_col.index
    
    min_col = rms_calib.iloc[:, col].nsmallest(3) #finds the 3 smallest values of each column
    i_min_col = min_col.index
    
    mean_max_col = max_col.mean() #mean of the 3 highest values
...
#myfct.plot_emgDF(normDF_calib.iloc[:910000,:])
